...
class CountValidation:
    ...
    def load_template(self):
        env = Environment(loader=FileSystemLoader('ora2pg_count_validator'))
    ...
    def _ora2pg_count_generator(self,template,project_name,input_status):
        input_file_name = r"test_count_ora2pg.txt"
        ...
        output_file_name = r"ora2pg_count_validation.csv"
        html_file_name = r"ora2pg_count.html"
        out_dir_path = self.os_inst.dcg_ora_validation_report_path(project_name,output_file_name)
        html_dir_path = self.os_inst.dcg_ora_validation_report_path(project_name,html_file_name)
        ...
        if len(df) > 1:
            ...
            if len(data_df.index) != 0:
                ...
                self.logger.error(f"No Failed data present for ora2pg count")
                return False
    ...
    def status_pie_chart(self,project_name,merged_df):
        if len(merged_df.index) != 0:
            ...
            plt.axis('equal')
            plt.title('Ora2pg table count validation summary')
            file_name = r"count_status.png"
            out_dir_path = self.os_inst.dcg_ora_validation_report_path(project_name,file_name)
            plt.savefig(out_dir_path, format='png')
            plt.close()

    
    def ora2pg_count_generator(self,project_name,input_status):
        template = self.load_template()
        if self._ora2pg_count_generator(template,project_name,input_status):
            return True
        else:
            return False